...
export default async function CharitiesPage({
  ...
  // Initial Query
  let dbQuery = supabase.from("charities").select(`*, charity_events(*)`);
  ...
  const { data: charities } = await dbQuery.order("is_featured", {
    ascending: false,
  });
      ...
      <section className="bg-slate-900 dark:bg-black pt-32 pb-28 px-6 text-center border-b border-border">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-slate-400 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
            Your golf performance fuels these causes. 10% of your subscription
            is automatically distributed to your chosen partner.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-10">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <form className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full pl-16 pr-8 py-7 rounded-3xl border border-border bg-card shadow-2xl shadow-black/5 focus:ring-4 focus:ring-primary/10 outline-none text-lg font-medium"
            />
          </form>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
            {categories.map((cat) => (
              <a
                ...
                className={`px-6 py-4 rounded-2xl border text-sm font-bold whitespace-nowrap transition-all ${
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10 mt-16 pb-24">
          {charities?.map((charity) => (
            ...
            >
              {/* LOGO AREA*/}
              <div className="h-56 bg-muted/30 relative flex items-center justify-center p-12 overflow-hidden group">
                {charity.is_featured && (
                ...
                {charity.logo_url ? (
                  <div className="relative h-full w-full">
                    ...
                    <Image
                      ...
                      className="object-contain filter grayscale group-hover:grayscale-0 dark:brightness-90 dark:group-hover:brightness-100 transition-all duration-700 opacity-80 group-hover:opacity-100"
                    />
                  </div>
                ) : (
                ...
                {charity.charity_events?.length > 0 && (
                  <div className="mb-8 p-5 bg-muted/50 rounded-2xl border border-border">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                      <Calendar size={14} className="text-primary" /> Upcoming
                      Golf Days
                    </p>
